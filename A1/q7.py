...
def create_point_samples(model_path: str = "data/cow.obj", num_samples: int = 400, device: Optional[torch.device] = None) -> Pointclouds:
    """Creates point cloud by sampling points from mesh faces using area-weighted probability."""
    device = get_device() if device is None else device
    
    vertices, faces, _ = load_obj(model_path, device=device, load_textures=True, create_texture_atlas=True, texture_atlas_size=4)
    ...
    sampled_points = ((1 - u).unsqueeze(1) * selected_faces[:, 0] + (u * (1 - v)).unsqueeze(1) * selected_faces[:, 1] + (u * v).unsqueeze(1) * selected_faces[:, 2])
...
def render_points(point_cloud: Pointclouds, output_path: str, image_size: int = 256, background_color: List[float] = [1.0, 1.0, 1.0], device: Optional[torch.device] = None, fps: int = 15, rotation_step: int = 5, camera_distance: float = 5.0, camera_height: float = 10.0) -> List[np.ndarray]:
    ...
    for angle in tqdm(range(-180, 180, rotation_step), desc="Rendering frames"):
        R, T = look_at_view_transform(dist=camera_distance, elev=camera_height, azim=angle)
        cameras = FoVPerspectiveCameras(R=R, T=T, device=device)
        lights = PointLights(location=[[0, camera_height, -3]], diffuse_color=[[1.0, 0.9, 0.8]], device=device)
...
def render_mesh(output_path: str, image_size: int = 256, device: Optional[torch.device] = None) -> List[np.ndarray]:
    ...
    mesh = Meshes(verts=vertices, faces=faces, textures=TexturesVertex(textures)).to(device)
...
def create_comparison(output_dir: str = "output", sample_sizes: List[int] = [10, 100, 1000, 10000], image_size: int = 1024, device: Optional[torch.device] = None) -> None:
    ...
    for size in sample_sizes:
        ...
        frames = render_points(point_cloud=point_cloud, output_path=f"{output_dir}/q7_{size}.gif", image_size=image_size, device=device)
        all_frames.append(frames)
    
    mesh_frames = render_mesh(output_path=f"{output_dir}/q7_original.gif", image_size=image_size, device=device)
