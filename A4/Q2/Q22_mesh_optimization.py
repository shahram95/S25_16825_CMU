...
def optimize_mesh_texture(
    ...
    for i in tqdm(range(total_iter)):
        ...
        # Clip latents to avoid over saturation
        latents.data = latents.data.clip(-1, 1)
        ...
        if i % log_interval == 0 or i == total_iter - 1:
            ...
            # Optionally save intermediate mesh animations to monitor progress
            if i % (log_interval * 5) == 0 and save_mesh:
                render_360_views(
                    mesh.detach(),
                    renderer,
                    device=device,
                    output_path=osp.join(sds.output_dir, f"mesh_iter_{i}.gif"),
                )
