...
def optimize_an_image(
    ...
    for i in tqdm(range(total_iter)):
        ...
        if args.sds_guidance:
            loss = sds.sds_loss(latents, embeddings["default"], text_embeddings_uncond=embeddings["uncond"], guidance_scale=100, grad_scale=1)
        else:
            loss = sds.sds_loss(latents, embeddings["default"], text_embeddings_uncond=None, guidance_scale=1, grad_scale=1)
